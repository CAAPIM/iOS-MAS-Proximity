...
- (void)pollAuthorizationCode
{
    ...
    if (pollURL == nil || [pollURL length] == 0)
    {
        NSError *invalidURLError = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLEAuthorizationFailed errorDomain:MASProximityErrorDomainLocal];
  ...
  withParameters:nil
    ...
    responseType:MASRequestResponseTypeJson
      ...
      completion:^(NSDictionary *responseInfo, NSError *error) {
          ...
          if (error)
          {
              NSError * pollError = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLEAuthorizationPollingFailed info:responseInfo errorDomain:MASProximityErrorDomain];
              ...
              //
              if ([responseInfo objectForKey:MASProximityPKCEStateRequestResponseKey])
              {
                  NSString *responseState = [responseInfo objectForKey:MASProximityPKCEStateRequestResponseKey];
                  NSString *requestState = [MASAuthenticationProviders currentProviders].PKCEstate;
                  ...
                  //
                  else if (![[responseInfo objectForKey:MASProximityPKCEStateRequestResponseKey] isEqualToString:[MASAuthenticationProviders currentProviders].PKCEstate])
                  {
                      pkceError = [NSError errorInvalidAuthorization];
                  }
            ...
            if (![blockSelf.centralManager isPoweredOn])
            {
                ...
                @try {
                    ...
                    [blockSelf.centralManager scanForPeripheralsWithServices:@[
                                                                          [
                                                                           CBUUID UUIDWithString:blockSelf.serviceUUID]
                                                                          ]
                                                                options:@
                     {
                ...
                @catch (NSException *exception) {
                    ...
                    //
                    NSError *masError = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLEPeripheral info:exceptionInfo errorDomain:MASProximityErrorDomainLocal];
...
- (void)startScanningWithAuthenticationProvider:(MASAuthenticationProvider *)provider
{
    ...
    {
        NSError *invalidAuthError = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLEInvalidAuthenticationProvider errorDomain:MASProximityErrorDomainLocal];
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI
{
   //DLog(@"\n\ndiscovered peripheral: %@ for central:\n\n%@\n\n", peripheral.name, [self debugDescription]);
    ...
    {
        NSError *error = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLERSSINotInRange errorDomain:MASProximityErrorDomainLocal];
...
- (void)centralManager:(CBCentralManager *)central didFailToConnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error
{
   //DLog(@"\n\nfailed to connect to peripheral: %@ for central:\n\n%@ with error:\n\n%@\n\n", peripheral.name, [self debugDescription], [error localizedDescription]);
    ...
    if (error)
    {
        ...
        //
        NSError *masError = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLEPeripheral info:error.userInfo errorDomain:MASProximityErrorDomainLocal];
   ...
   //DLog(@"\n\ncalled with peripheral: %@, services: %@ with error:\n\n%@\n\n", peripheral.name, peripheral.services, [error localizedDescription]);
    ...
    if(error)
    {
       //DLog(@"\n\nError discovering services:\n\n%@\n\n", [error localizedDescription]);
        ...
        // Re-create an error with proper framework error domain and error code.
        //
        NSError *masError = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLECentral info:error.userInfo errorDomain:MASProximityErrorDomainLocal];
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverIncludedServicesForService:(CBService *)service error:(NSError *)error
{
   //DLog(@"\n\n%@\n\n", [self debugDescription]);
    ...
    if (error)
    {
        ...
        // Re-create an error with proper framework error domain and error code.
        //
        NSError *masError = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLEPeripheralServices info:error.userInfo errorDomain:MASProximityErrorDomainLocal];
   ...
   //DLog(@"called with peripheral: %@\n\n%@\n\n with error:\n\n%@\n\n", peripheral.name, [self debugDescription], [error localizedDescription]);
    ...
    if(error)
    {
       //DLog(@"\n\nError when discovering service from peripheral: %@ characteristics:\n\n%@\n\n", service.peripheral.name, [error localizedDescription]);
        ...
        // Re-create an error with proper framework error domain and error code.
        //
        NSError *masError = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLEPeripheralCharacteristics info:error.userInfo errorDomain:MASProximityErrorDomainLocal];
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
   //DLog(@"\n\n%@\n\n", [self debugDescription]);
    ...
    if(error)
    {
        NSError *errorDetail = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLEPeripheralCharacteristics info:error.userInfo errorDomain:MASProximityErrorDomainLocal];
