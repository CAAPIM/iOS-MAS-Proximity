...
- (void)requestUserPermissionOnBLEForDeviceName:(NSString *)deviceName completion:(MASCompletionErrorBlock)completion
{
    ...
    //
        ...
        completion(NO, [NSError errorForProximityErrorCode:MASProximityErrorCodeBLEDelegateNotDefined errorDomain:MASProximityErrorDomainLocal]);
    }
}
            ...
            if (![blockSelf.peripheralManager isPoweredOn])
            {
                ...
                if (!blockSelf.transferCharacteristic)
                {
                    @try {
                        ...
                        //
                        blockSelf.transferCharacteristic = [[CBMutableCharacteristic alloc] initWithType:[CBUUID UUIDWithString:blockSelf.characteristicUUID]
                        ...
                        // Create service
                        //
                        CBMutableService *transferService = [[CBMutableService alloc] initWithType:[CBUUID UUIDWithString:blockSelf.serviceUUID]
                                                                                           primary:YES];
                        ...
                        [blockSelf.peripheralManager startAdvertising:@
                         {
                             CBAdvertisementDataServiceUUIDsKey : @[[CBUUID UUIDWithString:blockSelf.serviceUUID]]
                         }];
                    }
           ...
           //DLog(@"\n\nreceived EOM indicating the end of the authorization request\n\n");
            ...
            if(error != nil)
            {
               //DLog(@"\n\nFailed to load authorization request with error: %@\n\n", error);
                NSError *authorizationError = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLEAuthorizationFailed info:error.userInfo errorDomain:MASProximityErrorDomainLocal];
            ...
            [self requestUserPermissionOnBLEForDeviceName:[authorizationRequest objectForKey:@"device_name"] completion:^(BOOL completed, NSError *error) {
                ...
                if (error)
                {
                    ...
                    if (completed)
                    {
                        ...
                        if (providerURL == nil || [providerURL length] == 0)
                        {
                         ...
                         withParameters:nil
                           ...
                           responseType:MASRequestResponseTypeTextPlain
                             completion:^(NSDictionary *responseInfo, NSError *error) {
                                 ...
                                 if (error)
                                 {
                                     NSError * invalidURLError = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLEAuthorizationFailed info:responseInfo errorDomain:MASProximityErrorDomain];
                                     [self notifyErrorForBLEState:invalidURLError];
                                     ...
                                     if (!blockSelf.isSubscribed)
                                     {
                                         NSError *noSubscribedDeviceError = [NSError errorForProximityErrorCode:MASProximityErrorCodeBLECentralDeviceNotFound errorDomain:MASProximityErrorDomainLocal];
